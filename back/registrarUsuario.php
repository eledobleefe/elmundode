...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	$buscarUsuario = $conexion->prepare("SELECT * FROM usuario WHERE email = '$email' LIMIT 1");
	$buscarUsuario->bindParam(':email', $email, PDO::PARAM_STR);
	$buscarUsuario->execute();

	//si existe
	if($buscarUsuario->rowCount() == 1) {
		$array_respuesta['error'] = "Este email ya existe";
		$array_respuesta['registrado'] = false;
	} else {
		...
		$_SESSION['idUsuario'] = $nuevoUsuarioId;
		$_SESSION['nombreUsuario'] = $nombreUsuario;
		$_SESSION['email'] = $email;
		$_SESSION['rol'] = $rol;
		$array_respuesta['redireccion'] = 'crear_mundos.php';
		$array_respuesta['registrado'] = true;
	}
